...
│  • Sets (named bundles of connections)                       │
│  • Approvals (which AI apps can use which sets)              │
...
A session is a short-lived token that lets a frontend launch the Connect UI. It carries the request: which user, which Set or scopes, where to redirect after.

```typescript
const session = await passport.sessions.create({
  userId: 'user_abc',                 // your DB id for the end user
  set: 'founder-stack',               // request a named Set
  // OR explicit scopes:
...
- If the user already has a Passport: they sign in, see "Allow Acme AI to use your Founder Stack?", click yes.
...
const mcpUrl = passport.mcp.url({
  userId: 'user_abc',
  set: 'founder-stack',           // or apps: ['gmail', 'calendar']
  ttl: 3600,                       // seconds; URL is signed, time-bounded
});

// e.g. https://mcp.passport.so/v1/u/pp_27a4_b9c1f/s/founder-stack?token=...
```
...
1. Verifies the signed token, resolves `(passportId, set)`.
2. Reads the Set's connections + the current user's policy.
3. For each tool the Set permits, picks the right provider adapter.
...
```typescript
// Create
await passport.sets.create({
  userId: 'user_abc',
  name: 'founder-stack',
  displayName: 'Founder Stack',
  description: 'My day-to-day tools.',
  apps: ['gmail', 'calendar', 'slack', 'notion', 'linear'],
});

// Update
await passport.sets.update('founder-stack', {
  apps: ['gmail', 'calendar', 'slack', 'notion', 'linear', 'github'],
});

// Delete
await passport.sets.delete('founder-stack');
```

A Set is just a named pointer into the user's connections. It doesn't own credentials. Approving a Set for an AI product is what creates the actual permission grant.
...
- `pp`: passport id
- `set` or `scopes`: what's permitted
- `exp`: expiry (default 1 hour, configurable up to 24h)
- `nonce`: for replay protection

**Streamable HTTP** transport per MCP spec. SSE supported as fallback. Tools dynamically loaded based on the Set/scopes.
...
| `product.approved` | User approved your product for a Set |
...
# MCP URL
mcp_url = passport.mcp.url(user_id="user_abc", set="founder-stack")
```
...
The Passport data model (Connection, Set, Approval, AuditEntry, Policy) ships as an open JSON schema. Anyone can:
...
GET  /v1/mcp/:passportId/:setName?token=...
  → MCP server: list_tools from Set's apps, route tool calls per Connection.provider
```

That's it. Five endpoints, one adapter, one Set type, one consent UI. Everything else is layering on top of those primitives.
...
2. **Per-action approval UX.** Some actions are read-only (low-risk), some are write (high-risk), some are destructive (delete). The Set model gives access; the policy model (TBD) gates *which* actions inside that scope require an in-loop approval. Designing this before v1 prevents painful migration later.
